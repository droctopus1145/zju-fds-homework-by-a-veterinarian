...
int if_in1(int num,int* cache,int front_c,int rear_c);
...
int main()
{
    ...
    for(i=0;i<M;i++)
    {
        ...
        if(index==-1 && rear_a-front_a<N && if_in1(temp,cache,front_c,rear_c))
        {
        ...
        else if(index==-1 && rear_a-front_a==N && if_in1(temp,cache,front_c,rear_c))
        {
        ...
        else if(index>=0 && if_out(temp,access,front_a,rear_a,K)==0 && if_in1(temp,cache,front_c,rear_c))
        {
            int temp1=access[index].pick;
            delete(temp,access,front_a,rear_a);
            access[rear_a-1].item=temp;
            access[rear_a-1].pick=temp1;
        }
        else if(index>=0 && if_out(temp,access,front_a,rear_a,K)==1 && if_in1(temp,cache,front_c,rear_c))
        {
        ...
        else if(if_in1(temp,cache,front_c,rear_c)>=0)
        {
            int i;
            for(i=if_in1(temp,cache,front_c,rear_c);i<rear_c;i++)
                cache[i]=cache[i+1];
            cache[rear_c-1]=temp;
        } 
        //printf("%d %d %d\n",temp,front_a,rear_a); 
